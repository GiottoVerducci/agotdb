...
namespace NRADB.Forms
{
    ...
    public partial class DeckBuilderForm : Form
    {
        ...
        private NraVersionedDeck _versionedDeck;
        private NraVersionedDeck _lastLoadedDeck; // used to detect changes in the versioned deck
        private NraDeck _currentDeck; // deck currently displayed
        private String _currentFilename; // filename of the deck (non-empty if the deck was loaded or saved)
        private readonly List<NraCardTreeView> _treeViews;
        ...
        private void TreeViewDeck_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                case Keys.Delete:
                    if (IsCardNode(node))
                        SubtractCardFromCurrent((NraCard)node.Tag); // keeps the coherence between the versioned deck and its display
                    break;
                case Keys.Insert:
                    if (IsCardNode(node))
                        AddCardToCurrent((NraCard)node.Tag); // keeps the coherence between the versioned deck and its display
                    break;
            }
        }
        ...
        private void TreeViewDeck_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (!IsCardNode(e.Node) || (e.Node.Tag == null))
                return;
            UpdateCardInformations((NraCard)e.Node.Tag);
        }
        ...
        /// </summary>
        private void UpdateCardInformations(NraCard card)
        {
            UpdateCardText(card);
            if (UserSettings.DisplayImages)
        ...
        /// </summary>
        private void UpdateCardText(NraCard card)
        {
            rtbCardText.Clear();
        ...
        /// </summary>
        private void UpdateCardImage(NraCard card)
        {
        ...
        private void UpdateTreeViewWithCards(NraCardTreeView treeView, NraCardList cards, NraDeck deck)
        {
        ...
        private TreeNode AddTypeNode(TreeView treeView, Int32 type)
        {
            ...
            if (order == -1) // type not found in the order list
                typeNode = treeView.Nodes.Add(type.ToString(CultureInfo.InvariantCulture), NraCard.GetTypeName(type)); // add it to the end
            else
            {
                ...
                typeNode = treeView.Nodes.Insert(i, type.ToString(CultureInfo.InvariantCulture), NraCard.GetTypeName(type));
            }
            typeNode.Tag = new TypeNodeInfo(type);
            return typeNode;
        }

        private void AddCardToTreeView(NraCardTreeView treeView, NraCard card, bool alreadyPresent)
        {
            treeView.BeginUpdate();
            ...
            if (!alreadyPresent)
            {
                var cardType = card.Type != null ? card.Type.Value : (Int32)NraCard.CardType.Unknown; // we may have no type for unknown cards
                ...
                var i = 0;
                while ((i < root.Nodes.Count) && (card.CompareTo((NraCard)root.Nodes[i].Tag) < 0))
                    ++i;
        ...
        private void SubtractCardFromTreeView(NraCardTreeView treeView, NraCard card, bool wasLastCopy)
        {
            treeView.BeginUpdate();
            ...
            var i = 0;
            while ((i < root.Nodes.Count) && (card.UniversalId != ((NraCard)root.Nodes[i].Tag).UniversalId))
                ++i;
        ...
        {
            var treeView = (NraCardTreeView)sender;
        ...
        public bool AddCardToDeck(NraCard card)
        {
            if (!_currentDeck.Editable)
                return false;

            NraCard c = _currentDeck.CardLists[1].AddCard(card); // TODO : modify to handle multiple deck lists
            AddCardToTreeView(treeViewDeck, c, c.Quantity > 1);
            return true;
        }

        private bool SubtractCardFromDeck(NraCard card)
        {
            if (!_currentDeck.Editable)
                return false;

            NraCard c = _currentDeck.CardLists[1].SubtractCard(card); // TODO : modify to handle multiple deck lists
            SubtractCardFromTreeView(treeViewDeck, card, c == null);
            return true;
        }

        public bool AddCardToSide(NraCard card)
        {
            if (!_currentDeck.Editable)
                return false;

            NraCard c = _currentDeck.CardLists[0].AddCard(card); // TODO : modify to handle multiple deck lists
            AddCardToTreeView(treeViewSide, c, c.Quantity > 1);
            return true;
        }

        private bool SubtractCardFromSide(NraCard card)
        {
            if (!_currentDeck.Editable)
                return false;

            NraCard c = _currentDeck.CardLists[0].SubtractCard(card); // TODO : modify to handle multiple deck lists
            SubtractCardFromTreeView(treeViewSide, card, c == null);
            return true;
        }

        public bool AddCardToCurrent(NraCard card)
        {
        ...
        private bool SubtractCardFromCurrent(NraCard card)
        {
            return tabControlDecks.SelectedTab == tabPageDeck
        ...
        private bool SaveVersionedDeck(bool forceCallToSaveDialog)
        {
            ...
            if (result == DeckSaveResult.Success)
                _lastLoadedDeck = (NraVersionedDeck)_versionedDeck.Clone(); // performs a deep copy
            else
            {
        ...
        private void LoadVersionedDeck()
        {
            ...
            var versionedDeck = new NraVersionedDeck();
        ...
        {
            _versionedDeck = new NraVersionedDeck();
        ...
        private static String TreeViewToText(NraCardTreeView treeView)
        {
    ...
    internal delegate bool CardOperation(NraCard card);
