...
namespace NRADB.Services
{
	public static class ProxyPdfGenerator
	{
		...
		public static IList<string> CreateProxiesPdf(string proxyFileName, NraDeck deck)
		{
        ...
        private static void AddProxiesToPrintList(IList<ProxyInfo> printList, IList<string> unprintedList, IEnumerable<NraCard> cardList)
		{
			foreach (var card in cardList)
			{
				...
				if (CardImageService.GetImageAvailability(cardfilepath) == ImageAvailability.Available)
					printList.Add(new ProxyInfo
					{
                        ...
                        MustRotate = false//card.Type != null && card.Type.Value == (int)NraCard.CardType.Plot
					});
				else
				{
