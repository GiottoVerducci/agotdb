...
namespace NRADB.Forms
{
    ...
    public partial class MainForm : Form
    {
        ...
        private NraCard _displayedCard;
        ...
        public MainForm(bool isMainForm = false)
        {
            ...
            if (isMainForm)
            {
                ...
                ApplicationSettings.DatabaseManager = new NraDatabaseManager("AGoT.mdb", "AGoTEx.mdb");
        ...
        {
            var entries = new List<NraCard>();
            for (var i = 0; i < dataGridView.Rows.Count; ++i)
                entries.Add(new NraCard(((DataRowView)dataGridView.Rows[i].DataBoundItem).Row));

            using (var sw = new StreamWriter(fileName, false, System.Text.Encoding.Default))
            {
                foreach (NraCard card in entries)
                {
        ...
        {
            //DownloadService.DownloadFile("http://NRADB.googlecode.com/files/NRADB%20-%20EN%20-%20Beta%200.722.zip", "c:\\temp\\toto.zip");
        ...
        {
            _displayedCard = new NraCard(row);
